...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CapsuleColliderPositioner : StateBehavior
    {
        ...
        [Tooltip("The adjustment to apply to the collider's radius.")]
        [SerializeField] protected float m_RadiusAdjustment;
        ...
        public void UpdateRotationHeight()
        {
            ...
            if (m_AdjustHeight) {
                ...
                if (m_PositionBone != null) {
                    m_Transform.position = MathUtility.TransformPoint(m_PositionBone.position + m_CenterOffset, m_Transform.rotation, -m_CapsuleCollider.center);
                }
        ...
        /// <summary>
        /// Adjusts the collider's radius to the specified value.
        /// </summary>
        /// <param name="targetOffset">The desired offset value.</param>
        private void AdjustRadiusAdjustment(float targetOffset)
        {
            var delta = targetOffset - m_RadiusAdjustment;
            m_CapsuleCollider.radius += delta;
            m_RadiusAdjustmentEvent = null;
            ...
            if (Physics.OverlapCapsuleNonAlloc(firstEndCap, secondEndCap, m_CapsuleCollider.radius * MathUtility.ColliderScaleMultiplier(m_CapsuleCollider),
                                    m_OverlapColliders, m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore) > 0) {
                m_CapsuleCollider.radius -= delta;
                m_RadiusAdjustmentEvent = Scheduler.Schedule(Time.fixedDeltaTime, AdjustRadiusAdjustment, targetOffset);
            } else {
                m_RadiusAdjustment = targetOffset;
            }
            m_CharacterLocomotion.EnableColliderCollisionLayer(collisionLayerEnabled);
