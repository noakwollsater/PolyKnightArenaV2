    ...
    public class CharacterLocomotion : StateBehavior, IForceObject
    {
                ...
                if (hitCount > 0) {
                    for (int i = 0; i < hitCount; ++i) {
                        ...
                        if (Vector3.Dot(closestRaycastHit.normal, m_Up) > 0 &&
                            (Physics.Raycast(MathUtility.TransformPoint(closestRaycastHit.point, m_Rotation, Vector3.forward * m_SkinWidth) + m_Up * m_MaxStepHeight,
                                                        -m_Up, out var raycastHit,
                                                        m_MaxStepHeight + m_SkinWidth, m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore) && Vector3.Dot(raycastHit.normal, m_Up) > 0.99f)) {
                            continue;
                        }
                        ...
                        // Stop moving if a ceiling is near the character or the slope is too steep.
                        if ((closestRaycastHit.distance < movementMagnitude || slopeAngle > m_SlopeLimit) && Mathf.Abs(Vector3.Dot(closestRaycastHit.normal, -m_Up)) > 0.5) {
                            minDistance = 0;
                            ...
                            if (closestRaycastHit.distance < movementMagnitude && (movementDot = Vector3.Dot(normalizedTargetMovement, projectedNormal)) > -0.99f && Vector3.Dot(m_DesiredMovement.normalized, -m_Up) < 0.99f) {
        ...
        private void DetectGroundCollision(bool sendGroundedEvents = true)
        {
            ...
            var hitCount = CombinedCast(targetPosition, m_GravityDirection,
                                        ...
                                        Mathf.Abs(localDesiredMovement.y) + m_SkinWidth + castOffset + slopeOffset + c_ColliderSpacing);
            if (hitCount > 0) {
                for (int i = 0; i < hitCount; ++i) {
                    var closestRaycastHit = QuickSelect.SmallestK(m_CombinedCastResults, hitCount, i, m_CastHitComparer);
                    var activeCollider = m_ColliderCount > 1 ? m_Colliders[m_ColliderIndexMap[closestRaycastHit]] : m_Colliders[m_ColliderIndex];
